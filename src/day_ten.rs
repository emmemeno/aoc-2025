...
struct Machine {
    ...
    buttons: Vec<Button>,
    // how many trigger for joltage are repeated in buttons
    joltage_triggers: HashMap<Trigger, u8>
}
...
impl Machine {
    fn from_str(input: &str) -> Self {
        ...
        // println!("Lights: {}", &light_str[1..]);
        // println!("Buttons: {}", &buttons_str.trim());
        // println!("Joltage: {}", &joltage_str[..joltage_str.len()-1]);

        let mut target_lights: Vec<bool> = vec![];
        ...
        let mut target_joltage: Vec<u16> = vec![];
        let joltage_str = &joltage_str[..joltage_str.len()-1];
        for j in joltage_str.split(",") {
            target_joltage.push(j.parse::<u16>().unwrap());
        }
        // println!("Target Lights: {target_lights:?}");
        // println!("Buttons: {buttons:?}");
        // println!("Joltage Requirement: {joltage_req:?}");
        Self { target_lights, target_joltage, buttons, joltage_triggers}
    }
    ...
    fn most_beneficial_buttons_for_light(&self, target: &[bool]) -> Vec<(ButtonId, Score)> {
        // println!("Most beneficial buttons for target: {target:?}");
        let light_to_switch: HashSet<u8> = target.iter().enumerate().filter(|(_, l)| **l).map(|(n, _)| n as u8).collect();
        ...
        for (n, button) in self.buttons.iter().enumerate() {
            ...
            let score = button.iter().filter(|&trigger| light_to_switch.contains(trigger)).count() as i32;
            button_ladder.push((n, score));
        }
        button_ladder.sort_by_key(|&(_, count)| -(count as i16));
        // give priority to larger buttons (on equal arm)
        button_ladder.sort_by(|(a_id, a_score), (b_id, b_score)|
            match b_score.cmp(a_score) {
                std::cmp::Ordering::Less => std::cmp::Ordering::Less,
                std::cmp::Ordering::Equal => {
                    self.buttons[*b_id].len().cmp(&self.buttons[*a_id].len())  
                },
    ...
    fn apply_button_to_lights(&self, button: ButtonId, target: &[bool]) -> Vec<bool> {
    ...
    fn button_light_combo_only_length(&self, actual_combo_length: usize, actual_target: &[bool], ladder: &mut Vec<Vec<Button>>, shortest_sequence: &mut usize) -> usize {
        ...
        for (_n, (button_id, _)) in mbb.into_iter().enumerate() {
            ...
            if all_light_are_off(&new_target) {
                ...
                self.button_light_combo_only_length(actual_combo_length + 1, &new_target, ladder, shortest_sequence);
            }
        }
        *shortest_sequence
    }

    fn button_light_combo_verbose(&self, actual_combo: &Vec<Button>, actual_target: &[bool], ladder: &mut Vec<Vec<Button>>, shortest_sequence: &mut usize) -> Vec<Vec<Button>> {
        let mbb = self.most_beneficial_buttons_for_light(&actual_target); 
        for (_n, (button_id, _)) in mbb.into_iter().enumerate() {
    ...
    fn most_beneficial_buttons_for_joltage(&self, target: &[u16]) -> Vec<(ButtonId, Score)> {
        ...
        for (n, button) in self.buttons.iter().enumerate() {
            if button.iter().any(|pos| {
                target[*pos as usize] == 0
                }) {
    ...
    fn button_joltage_combo_only_length(&self, actual_combo_length: usize, actual_target: &[u16], shortest_sequence: &mut usize, max_deep_search: usize) -> usize {
        ...
        for (_n, (button_id, _)) in mbb.into_iter().enumerate() {
            ...
            if all_joltage_are_zero(&new_target) {
                ...
                println!("Found a new top score ({}) combination for {:?}", actual_combo_length, self.target_joltage);
                break;
            } else if actual_combo_length + 2 < *shortest_sequence {
                self.button_joltage_combo_only_length(actual_combo_length + 1, &new_target, shortest_sequence, max_deep_search);
            }
        }
        *shortest_sequence
    }
...
[#####] (0,1,3) (0,3,4) (0,4) (2,3,4) (0,1,2) (0,1,3,4) (1,2,3,4) {17,19,27,35,35}".to_string();
        }
        InputMode::Normal => {
...
pub fn part_one() {
    ...
    for (n, m) in machines.iter().enumerate() {
        let single_output = m.button_light_combo_only_length(0, &m.target_lights, &mut vec![], &mut max_deep_search.clone());
...
pub fn part_two() {
    ...
    let test = 0;
    let best_button = machines[test].most_beneficial_buttons_for_joltage(&machines[test].target_joltage);
    println!("Best button for Joltage, machine #{test}:");
    println!("{:?} - {:?}", machines[test].buttons, machines[test].target_joltage);
    for (b_id, score) in best_button {
        println!("Button: {:?} - score: {}", machines[test].buttons[b_id], score);
    }
    let single_output = machines[test].button_joltage_combo_only_length(0, &machines[test].target_joltage, &mut 1000, max_deep_search);
    // for (n, m) in machines.iter().enumerate() {
